...
var first_slice, last_slice, num_slices, step;
...
function trackCells(start_slice, end_slice)
{
	...
	for(slice = start_slice; slice!=end_slice+step; slice+=step)
	{
		...
		if(step>0) showProgress(slice-first_slice, num_slices);
		else showProgress(last_slice-slice, num_slices);
	}
...
function initialize(start_slice,end_slice)
{
	...
	filterCellCoordinates(x_init, y_init);
...
function filterCellCoordinates(x_init,y_init)
{
	if(step < 0)
	{
		n = x_init.length;
...
function verifyNextStatus(cell_id)
{
	...
	{
		intensity = getResult("Mean",0);
		if(step>0 && intensity < ACTIVE_INTENSITY) return;
		if(step<0 && intensity > ACTIVE_INTENSITY) return;
		status_id++;
